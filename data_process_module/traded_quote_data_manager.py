...
class TradedQuoteDataManager:
    def __init__(self, config_file_path: str):
        ...
        self.quote_reader_dict: dict[
            (str, int, int, str), TradedQuoteReader
        ] = (
            {}
        )  # key: root, quote_date, expiration, quote_manager_id; value: TradedQuoteReader
    ...
    def request_data(
        ...
        quote_reader = self.quote_reader_dict.get(
            (root, date, expiration, quote_manager_id)
        )
        if start_msd != quote_reader.get_last_msd():
            raise ValueError(
                "start_msd should equal to last_msd in TradedQuoteReader, unless you are jumping ahead "
    ...
    def update_expirations(self, root: str, date: int, expiration_date_params: dict):
        ...
        func_params = dict(
            root=root, date=date, expiration_date_params=expiration_date_params
        )
        condition_params = {quote_folder_name: [root]}
        walk_in_process(
            root_system,
            [quote_folder_name, year, month],
            func,
            func_params,
            condition_params,
        )
    ...
    def _open_stream(
        ...
        """
        if self._check_stream_exist(
            root, date, expiration, quote_manager_id, soft_check=True
        ):
            raise ValueError(
                "reading stream already exists - use reset_reading_stream to reset the reading stream "
        ...
        path = generate_path(
            ...
            quote_type_folder=self.quote_folder_name,
            extension="csv",
        )
        quote_reader = TradedQuoteReader(
            ...
            expiration=expiration,
            asset_type="option",
            MSD_COL_NAME="ms_of_day",
        )  # TODO: Modify for supporting other asset types
        quote_reader.reset_msd(
            last_mds
        )  # set the quote_reader reading history to the last_mds
        self.quote_reader_dict[
            (root, date, expiration, quote_manager_id)
        ] = quote_reader

    def _close_stream(
        self, root: str, date: int, expiration: int, quote_manager_id: str
    ):
        """
        ...
        self.quote_reader_dict.pop(
            (root, date, expiration, quote_manager_id)
        )  # TODO: test the quote_reader is removed

    def request_close_stream(
        self, root: str, date: int, expiration: int, quote_manager_id: str
    ):
        self._close_stream(root, date, expiration, quote_manager_id)

    def request_open_stream(
        self,
        root: str,
        date: int,
        expiration: int,
        ...
        self._open_stream(
            ...
            date=date,
            expiration=expiration,
            quote_manager_id=quote_manager_id,
            last_mds=last_mds,
        )
