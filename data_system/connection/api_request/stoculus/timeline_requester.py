...
class StoculusTimelineRequester(StoculusRequester):
    ...
    def start_connection(self, data_hub: AssetDataHub):
        ...
        ticker_info = (
            StoculusRequestMaker.get_asset_data_hub_ticker_info_starter_request(
        ...
        data_hub.set_failed_tickers(
            failed_tickers=connection_data_json.get("failed_tickers", None)
        )
    ...
    def check_connection(self, data_hub: AssetDataHub):
        if not data_hub.has_token_key():
            raise ValueError(f"AssetDataHub {data_hub} is not connected")
        else:
            request_params = StoculusRequestMaker.get_token_status_request(
                public_token=data_hub.public_token, private_key=data_hub.private_key
            )
    ...
    def read_upto_time(self, data_hub: AssetDataHub, ms_of_day: int | float):
        if data_hub.public_token is None or data_hub.private_key is None:
            ...
            read_request = StoculusRequestMaker.get_read_upto_time_request(
                data_hub=data_hub, ms_of_day=ms_of_day
            )
        url = self.url(endpoints=["time", "next"])
        response = requests.get(url, params=read_request)
        return self.parse_response(response)

    def get_timeline_status(self, data_hub: AssetDataHub):
        if not data_hub.has_token_key():
            raise ValueError(f"AssetDataHub {data_hub} is not connected")
        request_params = StoculusRequestMaker.get_token_status_request(
            public_token=data_hub.public_token, private_key=data_hub.private_key
        )
        url = self.url(endpoints=["status", "tickers"])
        response = requests.get(url, params=request_params)
        return self.parse_response(response)
