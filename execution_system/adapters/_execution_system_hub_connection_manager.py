...
class ExecutionSystemHubConnectionManager:
    ...
    def __init__(self):
        self._execution_sessions: Dict[
            str, ExecutionSession
        ] = dict()  # key: execution_session_id, value: ExecutionSession
        self._hub_session_id_to_execution_session_id: Dict[
            str, str
        ] = dict()  # key: hub_session_id, value: execution_session_id
        self._hub_id_to_execution_session_ids: Dict[
            str, list
        ] = dict()  # key: hub_id, value: list of execution_session_ids

    def create_sessions(
        self, hub_id: str, hub_session_ids: List[str], operation_mode: OperationMode
    ):
        for hub_session_id in hub_session_ids:
            self._create_execution_session(
                hub_session_id=hub_session_id, operation_mode=operation_mode
            )

        for hub_session_id in hub_session_ids:
            execution_session_id = self.find_execution_session_id(
                hub_session_id=hub_session_id
            )
            if execution_session_id is None:
                raise ValueError(
                    f"Creation of execution session failed {hub_session_id}"
                )
            self._hub_id_to_execution_session_ids[hub_id].append(execution_session_id)

    def find_execution_session(
        self,
        hub_id: Optional[str] = None,
        hub_session_id: Optional[str] = None,
        execution_session_id: Optional[str] = None,
    ) -> None | list[ExecutionSession] | ExecutionSession:
        if (
            len(
                [
                    x
                    for x in [hub_session_id, execution_session_id, hub_id]
                    if x is not None
                ]
            )
            != 1
        ...
        if hub_id is not None:
            execution_session_ids = self._hub_id_to_execution_session_ids.get(
                hub_id, None
            )
            ...
            for execution_session_id in execution_session_ids:
                execution_session = self._execution_sessions.get(
        ...
        if hub_session_id is not None:
            execution_session_id = self._hub_session_id_to_execution_session_id.get(
                hub_session_id, None
            )
            return self.find_execution_session(
    ...
    def find_execution_session_id(
        self, hub_id: Optional[str] = None, hub_session_id: Optional[str] = None
    ) -> None | str | list[str]:
        """
        ...
        if hub_session_id is not None:
            return self._hub_session_id_to_execution_session_id.get(
                hub_session_id, None
            )
    ...
    def find_hub_session_id(self, execution_session_id: str) -> Optional[str]:
        for (
            hub_session_id,
            _execution_session_id,
        ) in self._hub_session_id_to_execution_session_id.items():
            if _execution_session_id == execution_session_id:
                return hub_session_id
        return None

    def _create_execution_session(
        self, hub_session_id: str, operation_mode: OperationMode
    ):
        execution_session = ExecutionSessionFactory.create_session(
            operation_mode=operation_mode
        )
        self.add_execution_session(execution_session)
        self._hub_session_id_to_execution_session_id[
            hub_session_id
