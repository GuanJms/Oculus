...
class QuoteManager:
    def __init__(self, MSD_COL_NAME: str, frequency: int = 60000):
        ...
        self.qb_diclist: dict[str, List[QuoteBoard]] = {}
    ...
    def run_once(self, delta_time_ms: int = None):
        ...
        transactions = []
        for root in self.qb_tree.keys():
            exp_links = self.qb_tree.get(root)
            for expiration in exp_links.keys():
                transactions += self._request_data(start_msd=start_msd,
                                                   end_msd=end_msd,
                                                   root=root, date=self.quote_date,
                                                   expiration=expiration)
        self.transaction_queue.add_transactions(transactions)

    def _request_data(self, start_msd: int, end_msd: int, root: str, date: int, expiration: int) \
            -> List[Transaction]:
        # TODO: request data from the data manager
        header, transactions_raw = self.data_manager.request_data(start_msd=start_msd,
                                                                  end_msd=end_msd,
                                                                  root=root,
                                                                  date=date,
                                                                  expiration=expiration,
    ...
    def add_quote_board(self, quote_board: QuoteBoard):
        ...
        try:
            ...
            raise ValueError('Make sure the data manager is connected before adding quote board')
    ...
    def get_subscribed_quote_boards(self) -> List[QuoteBoard]:
    ...
    def initialize(self):
        # TODO: initialize the quote manager with the backtest params epsecially with the start quote_date and last_msd
