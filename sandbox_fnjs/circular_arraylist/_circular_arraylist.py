...
class CircularArrayList:
    ...
    def set_node_base(self, value: bool):
        if not isinstance(value, bool):
            raise Exception("Value must be a boolean")
        self._node_base = value
    ...
    def append(self, __value=None, **kwargs):
        if self._count == len(self._data):
            self.set_capacity(self._count + 1)
        if not self._node_base:
            self._data[(self._start + self._count) & (len(self._data) - 1)] = __value
        else:
            self._data[(self._start + self._count) & (len(self._data) - 1)].set(**kwargs)
