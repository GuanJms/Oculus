...
from strategics.repo.decorator.option.selection.expiration.range_dte import RangeDTESignal
from strategics.repo.decorator.option.selection.expiration.single_dte import SingleDTESignalGenerator
...
class ExpirationDTERule(RuleDecorator):
    ...
    def signal_generator(self) -> List[SignalGenerator]:
    ...
    def _generate_single_dte_signal(self) -> List[SignalGenerator]:
        single_dte_signal = SingleDTESignalGenerator(self._dte)
        return [single_dte_signal]

    def _generate_range_dte_signal(self) -> List[SignalGenerator]:
