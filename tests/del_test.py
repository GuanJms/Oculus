...
def walk_in_process(
    path, folders, func, func_params, condition_params
):  # TODO: add hint for the input
    if len(folders) == 0:
        ...
        for root_path, dirs, files in os.walk(path):
            if folders[0] in dirs:
                if folders[0] in condition_params:
                    ...
                    if (
                        prev_dir not in condition_params[folders[0]]
                    ):  # folder[0] is the next target folder
                        continue
                walk_in_process(
                    os.path.join(root_path, folders[0]),
                    folders[1:],
                    func,
                    func_params,
                    condition_params,
...
roots = ["SPY", "SPX"]
root_expirations = {
    "SPY": {"min_expiration": 20240105, "max_expiration": 20240307},
    "SPX": {"min_expiration": 20240105, "max_expiration": 20240110},
}
year = str(date)[:4]
month = str(date)[4:6]
...
func_params = dict(
    date=date, root_expirations=root_expirations, expiration_dict=expiration_dict
)
condition_params = {quote_folder_name: roots}
walk_in_process(
    root_system, [quote_folder_name, year, month], func, func_params, condition_params
)
